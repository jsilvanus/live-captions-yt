...
| `/cues` | `CuesPage` | Cue rules CRUD editor (`GET/POST/PUT/DELETE /cues/rules`), scoped to `phrase`/`regex`/`section`/`fuzzy` rule types — linked from the Assets page's "Global cues" card (plan_cues.md Phase 10). Rules with other match types (`semantic`, `event_cue`, sound-cue types, future `composite`) list/toggle/delete but lock their edit form. Thin wrapper around `CuesManager` (embeddable core, `embedded` prop — also used by the Planner's `PlannerAssistPanel`). |
