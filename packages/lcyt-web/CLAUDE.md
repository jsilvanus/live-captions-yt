...
- `hooks/` — Custom React hooks: useActiveBroadcast, useBrowserFileSaving, useDashboardConfig, useEscapeKey, useEventStream, useFileStore, useGuidedAction, useProjectFeatures, useSentLog, useSession, useToast, useUserAuth, useVariables, useWebSpeech, useWindowEvent. Notably: `useSession` (`BackendCaptionSender` session lifecycle hook; `onConnected` payload includes `token`), `useDashboardConfig` (dashboard panel/layout CRUD, localStorage persistence), `useWebSpeech` (WebSpeech recognition state machine: start, stop, error recovery), `useProjectFeatures` (project feature flag hook), `useUserAuth` (user authentication hook), `useEventStream` (shared authenticated `/events/stream?flat=1` EventSource multiplexer), `useVariables` (`{{ }}` variable snapshot: `GET /variables` + shared `/events/stream` topic subscription + `POST /variables/refresh`, from `lcyt-connectors`). `useActiveBroadcast` (shared `GET /broadcasts/active` snapshot — broadcast + projectName; re-fetches on the `lcyt:active-broadcast-changed` window event via `notifyActiveBroadcastChanged()`, fired by every active-broadcast/status mutation site; plan/broadcasts_next). `useGuidedAction.jsx` (plan_ai_roles_framework.md's dialog-driving primitive, formerly unbuilt — see `CONSIDER.md`'s note on Asset Control's gap): `GuidedActionProvider` + `useGuidedActionTargets(handlers)` (a dialog-owning component registers `{ toolName: (args) => {...} }` handlers that open/pre-fill its own Add/Edit/Delete dialog) + `useGuidedActionDispatcher()` (returns `dispatch(toolName, args) => boolean`, used by `components/agent/RoleAssistantPanel.jsx` to route a `confirm`-mode staged tool call from `POST /roles/:roleCode/message` to the matching dialog). Registered today by `setup-hub/CaptionTargetsSection.jsx`/`CameraSection.jsx`/`MixerSection.jsx` for `caption_target.*`/`camera.*`/`mixer.*` (each manager's `TargetCaptionsPage.jsx`/`ProductionCamerasPage.jsx`/`ProductionMixersPage.jsx` exposes `openAddPrefilled`/`openEditPrefilled`/`openDeleteConfirm` via `useImperativeHandle`, alongside the pre-existing `openAdd`).
...
**Added 2026-07-21 (Vitest):**
- `test/components/RoleAssistantPanel.test.jsx` (5 tests) — `RoleAssistantPanel.jsx` (plan_ai_roles_framework.md's remaining Tier 1 item, Setup/Asset Control Assistant frontend): disabled state when not connected, `POST /roles/:roleCode/message` request shape + rendering the plain-text reply, dispatching a staged `pendingActions` tool call to a `useGuidedActionTargets()`-registered handler (`GuidedActionProvider` from `hooks/useGuidedAction.jsx`) and showing the "pre-filled... review and confirm" message, reporting an unhandled tool call as plain text instead of silently dropping it, and surfacing a non-ok response as an error.
