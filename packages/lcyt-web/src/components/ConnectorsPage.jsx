...
function ConnectorCard({ api, connector, onDeleted, onReload }) {
  const [expanded, setExpanded] = useState(false);
...
function VariablesPanel({ api }) {
  ...
  return (
    <div>
      {entries.length === 0 && <p style={{ opacity: 0.6, fontSize: '0.9em' }}>No variables yet.</p>}
      {entries.map(([name, v]) => (
...
/**
 * ConnectorsPage — `/connectors`. Manage API Connectors (outbound HTTP
 * endpoints), their nested Requests (method/path/query/body/response
 * mapping), and the resulting `{{name}}` variables.
 *
 * See docs/plans/plan_api_connectors_variables.md and
 * packages/plugins/lcyt-connectors/CLAUDE.md. Reached from the Setup Hub's
 * "API connectors" card.
 */
export function ConnectorsPage() {
  ...
  useEffect(() => { if (session.connected) load(); }, [session.connected, load]);
  ...
  if (!session.connected) {
    return (
      <div className="page-container" style={{ padding: '2rem' }}>
        <h2>🔌 API Connectors &amp; Variables</h2>
        <p style={{ opacity: 0.6 }}>Connect to a backend to configure API connectors.</p>
      </div>
    );
  }
  ...
  return (
    <div className="page-container" style={{ padding: '2rem', maxWidth: 900 }}>
      ...
      <p style={{ opacity: 0.7, marginBottom: '1.5rem' }}>
        Connect third-party services (calendars, ChMS, lighting consoles, weather, …) and expose
        their responses as <code>{'{{name}}'}</code> variables you can insert anywhere in your
        captions. Trigger a refresh from a file line with{' '}
        <code>{'<!-- !api:slug.slug -->'}</code> (on arrival), <code>{'<!-- api:slug.slug -->'}</code>{' '}
        (at send), or <code>{'<!-- api!:slug.slug -->'}</code> (prefetched, small blocking fallback).
      </p>

      <NewConnectorForm api={api} onCreated={load} />

      {connectors.map(c => (
        <ConnectorCard key={c.id} api={api} connector={c} onDeleted={deleteConnector} onReload={load} />
      ))}
      {connectors.length === 0 && <p style={{ opacity: 0.6 }}>No connectors yet — create one above.</p>}

      <h3 style={{ marginTop: '2rem' }}>Variables</h3>
      <p style={{ opacity: 0.7, fontSize: '0.9em' }}>
        Current resolved snapshot — manually-set values (<code>source: manual</code>) or the last
        value a connector request resolved (<code>source: connector</code>).
      </p>
