...
export function createInputBar(container, { captionView } = {}) {
  ...
  window.addEventListener('lcyt:line-send', async (e) => {
    ...
    try {
      ...
      if (e.detail.fileId !== undefined && captionView) {
        captionView.flashSent(e.detail.fileId, e.detail.lineIndex);
      }
    } catch (err) {
      handleSendError(err);
    }
  ...
  return { element: el, focus: () => input.focus(), triggerSend: handleSend };
}
