...
const TIER_B = [
  ...
  { key: 'app.allowed_domains', env: 'ALLOWED_DOMAINS', type: 'csv', default: ['lcyt.fi', 'www.lcyt.fi', 'localhost'], category: 'application', apply: 'hot', description: 'Domains allowed to open a caption session (CORS filter). `*` allows any domain.' },
  { key: 'app.allowed_rtmp_domains', env: 'ALLOWED_RTMP_DOMAINS', type: 'csv', default: null, category: 'application', apply: 'hot', description: 'Domains allowed to use the /stream relay endpoints. Falls back to Allowed Domains when unset.' },
  ...
  { key: 'app.login_rate_limit_max', env: 'LOGIN_RATE_LIMIT_MAX', type: 'int', default: 50, category: 'application', apply: 'hot', description: 'Max failed /auth/login + /auth/register attempts per IP per 15 minutes (0 disables).' },
  ...
  { key: 'media.cea708_offset_ms', env: 'CEA708_OFFSET_MS', type: 'int', default: 2000, category: 'media', apply: 'restart', description: 'CEA-708 caption timing offset in ms. Captured at module load — restart required.' },
  ...
  // --- Compute -----------------------------------------------------------
  { key: 'compute.ffmpeg_runner', env: 'FFMPEG_RUNNER', type: 'enum', enum: ['local', 'docker', 'worker'], default: 'local', category: 'compute', apply: 'restart', description: 'ffmpeg execution backend. Captured at composition time (factory choice) — restart required.' },
  ...
  { key: 'graphics.dsk_rtmp_app', env: 'DSK_RTMP_APP', type: 'string', default: 'live', category: 'graphics', apply: 'manager', description: 'RTMP application name for DSK renderer output.' },
