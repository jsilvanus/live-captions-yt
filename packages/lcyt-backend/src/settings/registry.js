...
const TIER_A = [
  ...
  { key: 'bootstrap.platform_credential_key', env: 'PLATFORM_CREDENTIAL_KEY', type: 'secret', default: '', secret: true, category: 'bootstrap', description: 'Base64 32-byte AES-256-GCM key encrypting broadcast-platform OAuth tokens at rest (lcyt-platforms). Tier A alongside JWT_SECRET because it decrypts refresh tokens. Unset = platform accounts cannot be connected (warns at startup, refuses at write — never a plaintext fallback). Rotation is not built: changing it invalidates every stored credential.' },
...
const TIER_B = [
  ...
  { key: 'app.youtube_client_id', env: 'YOUTUBE_CLIENT_ID', type: 'string', default: '', category: 'application', apply: 'hot', description: 'Google OAuth 2.0 Web client ID for the server-side YouTube OAuth flow (GET /platforms/youtube/oauth/start).' },
  { key: 'app.youtube_client_secret', env: 'YOUTUBE_CLIENT_SECRET', type: 'secret', default: '', secret: true, category: 'application', apply: 'hot', description: 'Google OAuth 2.0 client secret. Required alongside the client ID for the authorization-code exchange — the retired implicit flow needed no secret, the server-side flow does.' },
  { key: 'app.platform_oauth_redirect_base', env: 'PLATFORM_OAUTH_REDIRECT_BASE', type: 'string', default: '', category: 'application', apply: 'hot', description: "Base URL the platform OAuth callback is reached at, e.g. https://api.example.com. Must exactly match the redirect URI registered with the provider. Falls back to Backend URL, then Public URL." },
  { key: 'app.platform_stats_poll_interval_s', env: 'PLATFORM_STATS_POLL_INTERVAL_S', type: 'int', default: 30, category: 'application', apply: 'timer', description: 'How often to poll live viewer counts for broadcasts synced to an external platform. Floored at 5s — a server-side poll has no user-facing brake and burns provider API quota.' },
  ...
  { key: 'graphics.max_file_bytes', env: 'GRAPHICS_MAX_FILE_BYTES', type: 'int', default: 5 * 1024 * 1024, category: 'graphics', apply: 'hot', zeroIsInvalid: true, description: 'Max uploaded image size in bytes.' },
  { key: 'graphics.max_storage_bytes', env: 'GRAPHICS_MAX_STORAGE_BYTES', type: 'int', default: 50 * 1024 * 1024, category: 'graphics', apply: 'hot', zeroIsInvalid: true, description: 'Max total image storage per key in bytes.' },
  ...
  { key: 'ai.vision_preview_base_url', env: 'VISION_PREVIEW_BASE_URL', type: 'string', default: '', category: 'ai', apply: 'restart', description: "Base URL VisionFrameFetcher (Tracker/Describer roles) polls for preview JPEGs; falls back to http://localhost:$PORT. Not in the original lcyt-backend/CLAUDE.md table (documented only in lcyt-agent's own CLAUDE.md) — resolved once per VisionRoleManager.start() call via SettingsService, restart-tier since a running poll loop doesn't re-read it mid-session." },
