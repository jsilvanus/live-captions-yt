...
export class SessionStore {
  ...
  // ---------------------------------------------------------------------------
  // DSK SSE subscriber registry
  // ---------------------------------------------------------------------------

  /**
   * Register an SSE response as a DSK subscriber for an API key.
   * @param {string} apiKey
   * @param {import('express').Response} res
   */
  addDskSubscriber(apiKey, res) {
    if (!this._dskSubscribers.has(apiKey)) this._dskSubscribers.set(apiKey, new Set());
    this._dskSubscribers.get(apiKey).add(res);
  }

  /**
   * Remove a DSK subscriber.
   * @param {string} apiKey
   * @param {import('express').Response} res
   */
  removeDskSubscriber(apiKey, res) {
    const set = this._dskSubscribers.get(apiKey);
    if (!set) return;
    set.delete(res);
  ...
  /**
   * Emit a DSK event to all subscribers for an API key.
