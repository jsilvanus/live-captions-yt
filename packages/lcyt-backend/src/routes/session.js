 ...
 * @param {{ relayManager?: object, dskCaptionProcessor?: Function, soundCaptionProcessor?: Function, resolveStorage?: Function }} [opts]
 * @returns {Router}
 */
export function createSessionRouters(db, store, jwtSecret, auth, { relayManager = null, dskCaptionProcessor = null, soundCaptionProcessor = null, resolveStorage = null } = {}) {
  const router = Router();
  router.use('/live',     createLiveRouter(db, store, jwtSecret));
  router.use('/captions', createCaptionsRouter(store, auth, db, relayManager, dskCaptionProcessor, resolveStorage, soundCaptionProcessor));
