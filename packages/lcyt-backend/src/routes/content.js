 ...
 * @param {{ hlsManager?: object, hlsSubsManager?: object, sttManager?: object }} [managers]
 * @returns {Router}
 */
export function createContentRouters(db, auth, store, jwtSecret, { hlsManager = null, hlsSubsManager = null, sttManager = null } = {}) {
  const router = Router();
  router.use('/stats',           createStatsRouter(db, auth, store));
  router.use('/usage',           createUsageRouter(db));
  router.use('/file',            createFileRouter(db, auth, store, jwtSecret));
