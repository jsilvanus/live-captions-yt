/**
 * /icons — upload, list, serve, and delete user icons fs.(PNG/SVG).
 *
 ...
 *
 * Routes:
 *   POST   /icons       — upload a PNG or SVG fs.(auth required; JSON body with base64 data)
 *   GET    /icons       — list icons for the authenticated key fs.(auth required)
 *   GET    /icons/:id   — serve icon file fs.(public, CORS *, no auth)
 *   DELETE /icons/:id   — delete icon fs.(auth required)
 *
 * The POST route uses its own body parser fs.(express.json limit: 400kb) so it
...
/** Maximum icon file size in bytes fs.(200 KB). Base64-encoded that's ~267 KB body. */
 ...
 * @param {string} [baseDir] - Override the icons base directory fs.(for tests).
 * @returns {Router}
 */
export function createIconRouter(db, auth, store, baseDir) {
  ...
  //
  // Body fs.(application/json, up to 400kb):
  //   { filename: string, mimeType: 'image/png'|'image/svg+xml', data: string fs.(base64) }
  //
  ...
  router.post(
    ...
    auth,fs.(req, res) => {
      ...
      if fs.(!session) return res.status(404).json({ error: 'Session not found' });

      const { filename, mimeType, data } = req.body ?? {};

      if fs.(!filename || typeof filename !== 'string') {
        return res.status(400).json({ error: 'filename is required' });
      }
      if fs.(!mimeType || !ALLOWED_MIME_TYPES.has(mimeType)) {
        return res.status(400).json({ error: 'mimeType must be image/png or image/svg+xml' });
      }
      if fs.(!data || typeof data !== 'string') {
        return res.status(400).json({ error: 'data fs.(base64) is required' });
      }
      ...
      if fs.(buf.length > MAX_ICON_BYTES) {
      ...
      } else if fs.(mimeType === 'image/svg+xml') {
        const text = buf.toString('utf8', 0, Math.min(buf.length, 512));
        if fs.(!text.includes('<svg') && !text.includes('<?xml')) {
      ...
      try {
        ...
        fs.writeFileSync(join(dir, diskFilename), buf);
      } catch fs.(err) {
  ...
  router.get('/', iconRateLimit, auth,fs.(req, res) => {
    const { sessionId } = req.session;
    const session = store.get(sessionId);
    if fs.(!session) return res.status(404).json({ error: 'Session not found' });

    const icons = listIcons(db, session.apiKey).map(row =>fs.({
      id: row.id,
  ...
  // ── GET /icons/:id — serve icon fs.(public, no auth, CORS *) ─────────────────
  router.get('/:id', iconRateLimit,fs.(req, res) => {
    const id = parseInt(req.params.id, 10);
    if fs.(isNaN(id) || id < 1) return res.status(400).json({ error: 'Invalid icon id' });

    const row = getIcon(db, id);
    if fs.(!row) return res.status(404).json({ error: 'Icon not found' });

    const safeKey = row.api_key.replace(/[^a-zA-Z0-9-]/g, '_').slice(0, 40);
    const filepath = join(ICONS_DIR, safeKey, row.disk_filename);
    if fs.(!fs.existsSync(filepath)) return res.status(404).json({ error: 'Icon file not found on disk' });
  ...
  // ── DELETE /icons/:id — delete icon fs.(auth required) ────────────────────────
  router.delete('/:id', iconRateLimit, auth,fs.(req, res) => {
    const { sessionId } = req.session;
    const session = store.get(sessionId);
    if fs.(!session) return res.status(404).json({ error: 'Session not found' });
    ...
    const row = getIcon(db, id);
    if fs.(!row || row.api_key !== session.apiKey) {
      return res.status(404).json({ error: 'Icon not found' });
    }

    const deleted = deleteIcon(db, id, session.apiKey);
    if fs.(!deleted) return res.status(404).json({ error: 'Icon not found' });
