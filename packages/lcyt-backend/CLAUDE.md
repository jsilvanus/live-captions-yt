...
**Server settings (`plan_env_to_ui_settings.md`):** `src/settings/registry.js` is now the authoritative enumeration of every setting below — types, defaults, and an `apply` mode (`hot`/`timer`/`manager`/`restart`) documenting whether a DB-saved value (via `GET/PUT/DELETE /admin/server-settings`, Admin → Server in `lcyt-web`) takes effect immediately or needs a restart. Precedence is env > DB > registry default; env-set values show as "env-locked" in the admin UI. Where this table below and the registry disagree, the registry (and the actual read site) wins — this table is retained for now as a human-readable reference of the env-var names and their historical defaults, not re-derived per edit. Fully wired to `SettingsService` today: `lcyt-backend`'s own read sites (Application/Contact/Sessions & Retention categories, plus `RTMP_RELAY_ACTIVE`/`MUSIC_DETECTION_ACTIVE` route-mount gates — now request-time hot gates, not module-load `if`s), `lcyt-files` (`FILE_STORAGE`/`S3_*`), and `lcyt-music` (`MEDIAMTX_HLS_BASE_URL`, `HLS_LOCAL_RTMP`/`HLS_RTMP_APP`, `MUSIC_CLASSIFIER_URL`). **Not yet wired** (env-only in practice despite being registered): `lcyt-rtmp`'s and `lcyt-dsk`'s module-top-level constants, `lcyt-production`'s `MediaMtxClient` construction, and `lcyt-agent`'s embedding functions — see `CONSIDER.md` for why each was deferred.
