...
**Server settings (`plan_env_to_ui_settings.md`):** `src/settings/registry.js` is the authoritative enumeration of every setting below — types, defaults, and an `apply` mode (`hot`/`timer`/`manager`/`restart`) documenting whether a DB-saved value (via `GET/PUT/DELETE /admin/server-settings`, Admin → Server in `lcyt-web`) takes effect immediately or needs a restart. Precedence is env > DB > registry default; env-set values show as "env-locked" in the admin UI. Where this table below and the registry disagree, the registry (and the actual read site) wins — this table is retained as a human-readable reference of the env-var names and their historical defaults, not re-derived per edit. Wired to `SettingsService`: `lcyt-backend`'s own read sites (Application/Contact/Sessions & Retention categories, `metrics.project_labels`, `app.backend_url` for the perception-job dispatch base URL, `stt.*` request-body defaults in `routes/stt.js`, `retention.recording_upload_delay_ms`/`retention.session_ttl` in `routes/live.js`, plus `RTMP_RELAY_ACTIVE`/`MUSIC_DETECTION_ACTIVE` route-mount gates — request-time hot gates, not module-load `if`s), `lcyt-files` (`FILE_STORAGE`/`S3_*`), `lcyt-music` (`MEDIAMTX_HLS_BASE_URL`, `HLS_LOCAL_RTMP`/`HLS_RTMP_APP`, `MUSIC_CLASSIFIER_URL`), `lcyt-rtmp` (relay/crop/STT/radio/HLS/preview managers — CEA-708 timing is genuinely hot now), `lcyt-dsk` (image upload gate/limits, DSK RTMP + renderer URLs), `lcyt-production`'s `MediaMtxClient` and `production.camera_preview_base_url` (resolved once at `createProductionRouter()` construction, threaded into `captureCameraThumbnail()`'s opts), and `lcyt-agent`'s embedding functions plus `ai.vision_preview_base_url` (resolved once per `VisionRoleManager.start()` call, threaded into `VisionFrameFetcher`'s constructor opts) — server-default provider path; per-project `ai_config` still wins. **Not yet wired** (real, confirmed gaps — see `CONSIDER.md`): `lcyt-backend`'s own `storage/s3.js` (`S3_ENDPOINT`/`S3_BUCKET`/`S3_REGION`/`S3_ACCESS_KEY_ID`/`S3_SECRET_ACCESS_KEY` — no `settings` threaded through its call chain from `db/videos.js`) and `lcyt-cues`'s `CUE_EVENT_TIMEOUT_MS` (no registry entry exists yet). `lcyt-production`'s `CAMERA_THUMBNAILS_DIR` is deliberately Tier A/env-only (see `registry.js`'s `bootstrap.camera_thumbnails_dir`), not a gap.
...
| `WORKER_DAEMON_URL` | Worker daemon URL when `FFMPEG_RUNNER=worker`; also the fallback perception-job dispatch target (`plan_video_perception.md` Phase 2) when `ORCHESTRATOR_URL` is unset | none |
...
| `BACKEND_URL` | Server's own URL (used internally for DSK renderer etc.); also the base URL a remote perception worker (`ORCHESTRATOR_URL`/`WORKER_DAEMON_URL`) is given for `frameUrl`/`callbackUrl` — falls back to `lcyt-production`'s `DEFAULT_PREVIEW_BASE_URL` (`http://localhost:$PORT`) when unset, which is only correct for a same-host/dev setup, not a real remote worker | none |
...
| `ORCHESTRATOR_URL` | Orchestrator base URL for the burst-VM accounting poller (`/compute/burst/history`); also the preferred perception-job dispatch target (`plan_video_perception.md` Phase 2) — checked before `WORKER_DAEMON_URL` | unset (poller off) |
...
POST /production/cameras/:id/perception/start — start the fps30 tracker job for a dedicated-feed camera (Bearer token; plan_video_perception.md Phase 2; 503 if no ORCHESTRATOR_URL/WORKER_DAEMON_URL configured, 400 if the camera has no cameraKey)
POST /production/cameras/:id/perception/stop  — stop it (Bearer token)
GET  /production/cameras/:id/perception/status — running-job info for this camera, or null (Bearer token)
POST /production/perception/ingest — internal: a worker POSTs one camera's detection here; fans into the cue engine's track_state + World State's camera.track_state (X-Internal-Auth: BACKEND_INTERNAL_TOKEN if set, not a project JWT — the caller is a compute worker). A detection tagged with the shared-feed sentinel cameraId is re-tagged by the shared-feed resolver (Phase 3) before reaching the aggregator, or dropped (still 200) if no camera is resolved yet.
POST /production/perception/shared/start — start the one shared-feed perception job for this project (mixer-input-only cameras, Phase 3; Bearer token, same 503/error-code shape as the per-camera route)
...
GET  /roles/:roleCode/captures              — browse the Tracker/Describer capture ring buffer (last 20, newest first, no frame bytes) (Bearer token; plan_ai_observability.md Stage 1, AI Observability page)
GET  /roles/:roleCode/captures/:id/frame    — raw JPEG for one captured frame (Bearer token or ?token=)
POST /roles/:roleCode/captures/:id/replay   — prompt sandbox: re-run analyse() against a captured frame with { promptOverride }, never persisted (Bearer token)
...
GET  /scene/state                  — World State / Scene State snapshot for the project (plan_video_perception.md Phase 1 §2): { activeSpeaker, cameras, segmentGuess, updatedAt }, empty/idle until Phase 2/3 wire in real update handlers (Bearer token)
...
- `src/perception-aggregator.js` — `createPerceptionAggregator({ store, eventBus, sceneState })` → `ingest(apiKey, detection)` (`plan_video_perception.md` §1/Phase 2 Stream C): fans one per-camera detection into two distinct emissions — a project-level `track_state` event on `store.getByApiKey(apiKey).emitter` (`session.emitter.emit('event', { type: 'track_state', data: { labels, ts } })`, the union of labels across every currently-visible camera for that project — this is `lcyt-cues`'s existing, previously-inert `_attachTrackerListener()` contract, unchanged) and a per-camera `camera.track_state` on the `EventBus` + a `sceneState.getState(apiKey).cameras[cameraId]` update (feeds `lcyt-agent`'s World State). Deliberately wholesale-replaces the union each tick rather than emitting once per camera, which would let `CueEngine._trackerState`'s one-blob-per-project cache get clobbered by whichever camera polled last. Lives here, not in a plugin, because only `lcyt-backend` holds `store` (session emitter access) together with the cross-plugin `eventBus`/`sceneState` — same reasoning as `caption-fanout.js`. `clearProject(apiKey)` drops a project's tracked-cameras entry from the internal `byProject` Map — wired to `onKeyDeleted()` in `server.js` (code-review fix; this Map had no eviction otherwise).
- `src/shared-feed-resolver.js` — `createSharedFeedResolver({ db, registry, aggregator })` (`plan_video_perception.md` §1/Phase 3): subscribes to `productionRegistry`'s existing `onProgramChanged`/`onCameraPresetRecalled` plain-callback signals (the same ones `plan_vertical_crop.md` Phase 4's `CropManager.applyForSource()` already consumes — not promoted to a real EventBus event, this resolver just registers its own listener alongside that one rather than migrating it), resolves a mixer `inputNumber` to a `prod_cameras.id`, and tracks the currently-active camera per project. **Scoped by `mixer_id`** (code-review fix): `mixer_input` alone is only unique within one mixer, so the lookup prefers `WHERE mixer_input = ? AND mixer_id = ?` (the `mixerId` from the `onProgramChanged` payload) and only falls back to an unscoped `mixer_id IS NULL` legacy camera when no scoped match exists — a plain `WHERE mixer_input = ?` would let two different mixers/projects sharing the same input number resolve to the wrong camera. `tagSharedDetection(apiKey, detection)` re-tags a shared-feed job's sentinel-`cameraId` detection with the real camera, or returns `null` if none is resolved yet (dropped, not forwarded with a made-up id). When the active camera changes, immediately calls `aggregator.ingest()` with `visible: false` for the outgoing camera — confirmed absent, not silence, per the plan's explicit requirement.
- `src/routes/perception.js` — `createPerceptionRouter(aggregator, resolver, { perceptionManager, internalToken, auth })`: `POST /production/perception/ingest`, the callback target `lcyt-worker-daemon`'s perception jobs POST detections to, gated by `X-Internal-Auth: BACKEND_INTERNAL_TOKEN` (mirrors `lcyt-orchestrator`'s own `requireInternalAuth()`, reimplemented rather than imported since this package has no dependency on `lcyt-orchestrator`) — not a project JWT, since the caller is a compute worker, not a user session. A detection whose job plan carried `feedKind: 'shared'` (`cameraId: null` — a real typed field, not a sentinel `cameraId` string, code-review fix) is routed through `resolver.tagSharedDetection()` first (Phase 3); `cameraId` is otherwise required (400 without it). Also `POST /shared/start|stop`/`GET /shared/status` (project-scoped `opts.auth`), thin wrappers over `perceptionManager.startSharedFeed`/`stopSharedFeed`/`sharedFeedStatus` — the project-wide equivalent of `lcyt-production`'s per-camera `/production/cameras/:id/perception/*` routes.
