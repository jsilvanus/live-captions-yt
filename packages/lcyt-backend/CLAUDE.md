...
6. **Project-access middleware** (`middleware/project-access.js`, `createProjectAccessMiddleware(db, jwtSecret, { requiredScope, jwtOnly })`) gates the project-scoped routers. `req.auth` = `{ kind, projectId, projectRole, scopes?, … }`. Two surfaces:
   - **Our UI (JWT-only):** the project config/CRUD routers and the legacy per-plugin SSE routes (DSK, connectors/variables/actions, roles, cues, STT/targets/translation, ai/agent, mcp-tokens) are mounted with `jwtOnlyAuth` — session/user/project/device JWTs only. **External `lcytmcp_` tokens are rejected (403)** here; they use the event stream instead. (`/variables/events` and `/stt/events` were already JWT-only via their own `jwt.verify` on `?token=`.)
   - **External subscribers (scoped tokens):** only `GET /events/stream` accepts external tokens, gated on an exact `requiredScope: 'events:read'` and then narrowed per-topic by `tokenAllowsTopic`. `GET /events/topics` is public.
   This keeps a clean split: browsers/CLIs use JWT everywhere; programmatic subscribers get a single, scope-limited, read-only event stream.
