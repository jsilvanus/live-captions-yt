...
describe('GET /variables — external-token scope filtering', () => {
  let server, baseUrl;

  // Auth that emulates lcyt-backend's projectAuth for a scoped external token:
  // sets req.session.apiKey + req.auth = { kind:'external', scopes }.
  function scopedAuth(req, res, next) {
    req.session = { apiKey: 'key1' };
    const raw = req.headers['x-test-scopes'];
    req.auth = raw ? { kind: 'external', projectId: 'key1', scopes: JSON.parse(raw) } : { kind: 'external', projectId: 'key1', scopes: null };
    next();
  }
  ...
  before(async () => {
    ...
    const app = express();
    app.use(express.json());
    app.use('/variables', createVariablesRouter(db, scopedAuth, bus, engine, scheduler, JWT_SECRET));
    await new Promise((resolve) => { server = app.listen(0, () => resolve()); });
    baseUrl = `http://127.0.0.1:${server.address().port}`;
    // Seed two variables (no scopes header = full access on write).
    for (const [name, value] of [['section', 'Prayer'], ['hymn', 'Amazing Grace']]) {
      await fetch(`${baseUrl}/variables`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, value }) });
    }
  });

  after(() => new Promise((resolve) => server.close(resolve)));

  async function snapshot(scopes) {
    const headers = { 'Content-Type': 'application/json' };
    if (scopes !== undefined) headers['x-test-scopes'] = JSON.stringify(scopes);
    const res = await fetch(`${baseUrl}/variables`, { headers });
    return (await res.json()).variables;
  }

  test('full-access token (null scopes) sees every variable', async () => {
    const vars = await snapshot(null);
    assert.deepEqual(Object.keys(vars).sort(), ['hymn', 'section']);
  });

  test('a variable.section.changed-scoped token only sees section', async () => {
    const vars = await snapshot(['events:read', 'variable.section.changed']);
    assert.deepEqual(Object.keys(vars), ['section']);
    assert.equal(vars.section.value, 'Prayer');
  });

  test('variable.* sees all variables', async () => {
    const vars = await snapshot(['events:read', 'variable.*']);
    assert.deepEqual(Object.keys(vars).sort(), ['hymn', 'section']);
  });

  test('events:read alone (no topic pattern) imposes no restriction', async () => {
    const vars = await snapshot(['events:read']);
    assert.deepEqual(Object.keys(vars).sort(), ['hymn', 'section']);
  });

  test('a token scoped only to dsk.* sees no variables', async () => {
    const vars = await snapshot(['events:read', 'dsk.*']);
