...
if (process.env.RTMP_RELAY_ACTIVE === '1') {
  // `auth` here should be scopedAuth('rtmp')/createProjectAccessMiddleware,
  // not the plain session-only middleware — only the four config-shaped
  // sub-routers below (ingestion/stream/radio's /config pair/crop) consume
  // it at all; the public/callback ones never take an auth param. opts.requireSetup
  // (plan_project_roles.md) gates their writes at Setup tier; defaults to a
  // no-op passthrough when omitted.
  const routers = createRtmpRouters(db, scopedAuth('rtmp'), rtmp, { allowedRtmpDomains, requireSetup: requireProjectRole(db, 'setup') });
...
- `routes/crop.js` — `/crop/*` session-authenticated CRUD API for crop config, presets, preset sets, and live position updates. Writes require Setup-tier project role (`opts.requireSetup`, plan_project_roles.md) — reads stay open to any project member.
...
- `db/crop.js` — crop migrations + helpers: `crop_config` (aspect/output/transition/active set), `crop_preset_sets` (banks), `crop_presets` (normalised 0..1 positions), `crop_source_map` + `resolveCropPresetForSource()` (most-specific wins: camera+PTZ-preset > camera > mixer input; scoped to the active set). Manual delete cascades (FK enforcement is off repo-wide). `crop_source_map.camera_preset` holds an **opaque per-camera preset identifier**, stored via `toPresetKeyOrNull()`; `resolveCropPresetForSource()` compares it as a string on both sides so a pre-existing numeric-looking value still matches. It is **not** the preset's `.id` (the UUID `POST /production/cameras/:id/preset/:presetId` and `lcyt-production`'s camera adapters address the preset by) — it's the same key `lcyt-web`'s crop editor (`cameraPresetSources()`) binds a row to: a VISCA preset's numeric `presetNumber`, or an AMX preset's array index (AMX presets carry no number at all). `routes/cameras.js`'s preset-trigger route recomputes this same presetNumber-or-index key from `:presetId` before firing `registry.notifyCameraPresetRecalled()`, so `applyForSource()` receives the key the UI actually bound instead of the raw `.id` (which would never match — a real bug fixed 2026-07-20, see CONSIDER.md). `resolveCameraIdForMixerInput(db, mixerInput)` — cross-plugin `prod_cameras` lookup (same guarded `hasProdCamerasTable()` pattern as `resolveRelaySourceCameraKey()` below) used by `CropManager.applyForSource()` to turn a bare mixer-input switch into a cameraId.
...
- `routes/ingestion.js` — `GET/PATCH /ingestion/config`, `POST /ingestion/config/rotate` — self-service ingest status/enable/rotate (session Bearer). Nested `{ video, dsk }` shape: `video.enabled` flips `relay_allowed` (feature-gated on `ingest` when `FEATURE_GATE_ENFORCE=1`); `dsk.enabled` is read-only (`graphics_enabled`) and `PATCH .../dsk` is `501` until a real DSK-ingest gate exists (§2/§2a). Writes require Setup-tier project role (`opts.requireSetup`, plan_project_roles.md); GET stays open to any project member.
- `routes/stream.js` — `GET/POST/PUT/DELETE /stream` — RTMP relay egress slot management (domain allowlist enforced — `requireRtmpDomain` reads `req.session.domain`, only ever present on a legacy plain session JWT; a project-scoped JWT has none, so the check treats "no domain on this token" as not-this-throttle's-concern rather than always-deny). No cap on the number of slots (`plan_ingest_feeds.md` §1b removed the earlier 4-slot limit — `POST /stream` without a `slot` defaults to `MAX(slot)+1`; a future per-team quota is an explicit non-goal, not enforced here). Slots carry `sourceView: 'program'|'crop'` — crop-view slots fan out the `{key}-crop` vertical rendition via a MediaMTX runOnPublish hook on that path instead of the raw ingest (never part of CEA-708/transcode/DSK modes) — **and/or** an optional `sourceCameraId` (a `prod_cameras` row with a `camera_key`), which takes priority over `sourceView` when set: the slot fans out from that camera's own MediaMTX path instead (`db/relay.js`'s `resolveRelaySourceCameraKey()` validates it and joins `source_camera_key` into every read; `rtmp-manager.js`'s `start()` groups camera-sourced slots by `cameraKey` and registers one runOnPublish hook per distinct feed via `_startCameraSourceRelay()`/`_stopCameraSourceRelays()`, mirroring the crop mechanism). Writes require Setup-tier project role (`opts.requireSetup`, plan_project_roles.md).
- `routes/stream-hls.js` — `GET /stream-hls/:key/*` — HLS video+audio proxy (public, rate-limited).
- `routes/radio.js` — `GET /radio/:key/*` — audio-only HLS proxy (public, rate-limited); `GET/PUT /radio/config` — self-service Web Radio metadata (session Bearer, §3/§3a); resolves nginx-rtmp callback stream names through `resolveApiKeyFromIngestStreamKey()`. `PUT /config` requires Setup-tier project role (`opts.requireSetup`, plan_project_roles.md) via `requireAuthConfigured`'s chained check; `GET /config` stays open to any project member.
