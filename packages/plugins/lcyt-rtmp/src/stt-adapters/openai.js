...
export class OpenAiAdapter extends EventEmitter {
  ...
  async _sendWav(wav, timestamp) {
    ...
    try {
      ...
      throw new Error(`OpenAiAdapter (PCM): request failed: ${err.message}`);
    }

    if (!resp.ok) {
      const errBody = await resp.text().catch(() => '');
      throw new Error(`OpenAiAdapter (PCM): server error ${resp.status}: ${errBody}`);
    }

    let data;
    try { data = await resp.json(); } catch (err) {
      throw new Error(`OpenAiAdapter (PCM): invalid JSON: ${err.message}`);
    }
