...
- `agentic-turn.js` — the shared tool-calling loop for every `agentic_chat` role. `runAgenticTurn({ apiSettings, systemPrompt, messages, tools, callTool, apiKey, shouldExecute, maxIterations })`: OpenAI-compatible `tools`/`tool_calls` wire format; a whole model exchange is held back as `pendingActions` (never partially executed) the moment ANY of its tool calls fails `shouldExecute`. `defaultShouldExecute` (read-only tools only) is Production Assistant's gate; `makeDialogShouldExecute(mode)` (read-only always, destructive never, else per mode) is the chat-driven-dialog roles' gate — implements the hard rule that deletes always go through the confirm dialog regardless of mode. `resolveRoleProviderSettings(providerRow, modelName, { bridgeManager })` returns `null` for `deer`-kind providers (unimplemented, see Phase 4) and for a bridge-relayed provider (`bridge_instance_id` set) when no `bridgeManager` was injected; otherwise it returns `{ transport: 'bridge', bridgeManager, bridgeInstanceId }` settings. `invokeModelCall()` branches on `apiSettings.transport === 'bridge'` to dispatch the request via `bridgeManager.sendCommand(instanceId, { type: 'model_call', endpoint, headers, payload })` instead of a direct `fetch()` — every route below (`roles-chat.js`, `vision-roles.js`, `production-assistant.js`, `planner.js`) is constructed with the composition root's `productionBridgeManager`, so bridge-relayed inference works end-to-end for every `agentic_chat` role and both vision roles, not just direct providers.
...
- `production-assistant.js` — `ProductionAssistantManager`: Assistant's tool calls (`camera.preset`/`mixer.switch`/`crop.activate_preset`, all `destructiveHint`, plus the read-only `crop.list_presets` — see `ai-roles.js`'s `assistant` role `available_tools` and `lcyt-tools`' `tools/crop.js`, plan_vertical_crop.md §4) are always held back by the turn loop's default gate, so this manager decides what happens to the one proposed action per trigger — queue it (`confirm` mode, `assistant_suggestion` SSE event) or execute + audit it (`auto` mode, `assistant_action` SSE event, emitted *after* execution). `AUTO_COOLDOWN_FLOOR_MS` (3000) is hard-enforced only for `auto` mode; `confirm` mode honours a configured `cooldownMs` down to 0 (self-limiting — a human gates each suggestion). `confirmSuggestion`/`rejectSuggestion` for the pending queue.
...
- `vision-adapters/{openai,google,anthropic}-vision.js` — one `analyse(imageBuffers, promptText, opts)` interface per vendor, each wrapping the real multimodal API (OpenAI chat completions + `image_url` parts, Gemini `generateContent` + `inline_data`, Claude Messages API + base64 image blocks). All three route their HTTP call through `agentic-turn.js`'s `invokeModelCall()`, so any of them works bridge-relayed (`transport: 'bridge'`, `bridgeManager`, `bridgeInstanceId` passed through from the constructor) as well as direct — Google keeps its `?key=` query-param auth (passed as `invokeModelCall`'s `endpointPath`, not a bearer header) and Anthropic keeps its `x-api-key`/`anthropic-version` headers (passed via `invokeModelCall`'s `headers` option) under either transport. `vision-adapters/index.js`'s `createVisionAdapter(vendor, settings)` selects by vendor; `'custom'` defaults to the OpenAI-compatible adapter.
- `vision-role-manager.js` — `VisionRoleManager`, one class parameterized by role (`tracker`/`describer`): one running session per `(apiKey, roleCode)`, wiring a `VisionFrameFetcher`'s frames through the selected vision adapter and emitting `tracker_update`/`describer_update` on the shared `RolesBus`. Adapter errors are logged and skipped, never crash the polling loop. Both roles are strictly non-action — neither ever calls a camera/mixer tool (reserved for Assistant alone). Also maintains a bounded **capture ring buffer** per `(apiKey, roleCode)` (`plan_ai_observability.md` Stage 1 §2, AI Observability page): every poll's `{ id, ts, prompt, frame: Buffer, outputMode, jsonSchema, result: {text,json}|null, error }` is recorded (`CAPTURE_LIMIT = 20`, oldest evicted first — deliberately small/in-memory, not a persisted table), surviving `stop()`/`start()`. `getCaptures(apiKey, roleCode)` returns the list newest-first with `frame` stripped; `getCapture(...)` returns one full entry including the frame. `replay(apiKey, roleCode, captureId, { apiSettings, vendor, promptOverride })` (§3, the prompt sandbox) re-runs `adapter.analyse()` against a captured frame — a fresh adapter built from the caller-supplied provider settings, not the (possibly stopped) running session's — with an optionally edited prompt, returning `{ original, replay }` for diffing; never writes back to `harness_config` and never touches the poll loop, so it can't affect production inference sampling.
- `routes/vision-roles.js` — `POST /roles/:roleCode/start|stop`, `GET /roles/:roleCode/status`, restricted to `tracker`/`describer`. Events handled by `routes/roles-chat.js`'s generic route (see above). AI Observability additions (dev/admin-only, same auth pattern): `GET /roles/:roleCode/captures` (browse the ring buffer, no frame bytes), `GET /roles/:roleCode/captures/:id/frame` (raw JPEG, `Cache-Control: private, max-age=86400, immutable`), `POST /roles/:roleCode/captures/:id/replay { promptOverride }` (same `loadConfigOr503` provider-resolution gate as `/start` — replay needs a live adapter — then delegates to `VisionRoleManager.replay()`; 404 on an unknown capture id, 503 on an adapter/provider error).
