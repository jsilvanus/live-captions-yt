...
- `roles-bus.js` — `RolesBus`: SSE subscriber registry keyed by `${apiKey}:${roleCode}` (mirrors `lcyt-connectors`' `VariablesBus`). Used by every role's event stream.
- `routes/roles-chat.js` — `createRolesChatRouter`: `POST /roles/:roleCode/message` for the three chat-driven-dialog roles (`setup_assistant`, `asset_control_assistant`, `dsk_designer` — Planner excluded, see below) — runs one full turn synchronously, emits `tool_call_started`/`tool_call_result`/`staged_action`/`reply` as SSE events as it goes, and also returns the result directly in the response. **`GET /:roleCode/events` here is the one place every role's SSE stream is handled** — generic over any catalog role code except Planner (which never streams); Production Assistant's and the vision roles' own routers deliberately do *not* define a competing `/events` route, since Express would route `/roles/<x>/events` here first regardless (see `test/roles-mount-order.test.js`, an integration test that mounts every role router together in `server.js`'s real order — a unit test of one router in isolation can't catch this class of collision). Also exports `resolveToolAllowlist(role, harnessConfig, allTools)`, reused by `routes/production-assistant.js`.
