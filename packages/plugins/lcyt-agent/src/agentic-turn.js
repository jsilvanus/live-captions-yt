 ...
 * (non-bridge-relayed) Ollama providers. Bridge-relayed providers are not yet
 * supported here — see CONSIDER.md.
 */
 ...
 * @param {{ apiUrl: string, apiKey: string, model: string }} apiSettings
...
export async function callChatCompletionWithTools(apiSettings, messages, openAiTools) {
  const url = `${apiSettings.apiUrl.replace(/\/$/, '')}/v1/chat/completions`;
 ...
 *
 * Bridge-relayed and 'deer'-kind providers aren't supported by the turn loop
 * yet — the shared OpenAI-compatible wire format this loop speaks assumes a
 * directly-reachable HTTP endpoint; see CONSIDER.md.
 *
 ...
 */
export function resolveRoleProviderSettings(providerRow, modelName) {
  if (!providerRow || !providerRow.enabled) return null;
  if (providerRow.bridge_instance_id) return null;
