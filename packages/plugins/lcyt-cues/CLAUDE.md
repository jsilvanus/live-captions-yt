...
createTrackerCueListener({ store, engine }); // inert until something emits track_state — see below
...
- `cue-processor.js` — `createCueProcessor()`: strips `<!-- cue:label -->` metacodes from caption text, fires cue events on session emitter, triggers CueEngine automatic/inline/event-cue/composite-rule evaluation. `createSoundCueListener()`/`createTrackerCueListener()`: mirror-image session-emitter listeners for `sound_label`/`track_state` events (music/silence and tracker-state cue rules respectively). Nothing in this repo emits `track_state` yet — the fps30 tracker subsystem is a separate, unbuilt piece (`docs/plans/plan_cues.md` Phase 9 "Tracker-state leaves"); the listener is exported and ready but inert until a producer exists, the same relationship `createSoundCueListener()` has always had with `lcyt-music`.
