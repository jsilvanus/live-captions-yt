...
- `routes/files.js` — `createFilesRouter(db, auth, store, jwtSecret, resolveStorage, invalidateStorageCache, requireSetup)` → `GET /file`, `GET /file/:id` (supports `?offsetMs=±N` on `vtt` files to shift cue times on the fly; 400 on non-vtt or out-of-range values, bound ±24h — this route alone does its own raw JWT verify + `store.get(payload.sessionId)`, independent of `auth`, so `?token=` direct download links keep working), `DELETE /file/:id`, `GET/PUT/DELETE /file/storage-config`. Every other route reads `req.session.apiKey` directly rather than resolving a live session — `store` is only still a constructor param for the `GET /file/:id` token path above. `POST /file`'s `sessionId` attribution reads `req.auth?.sessionId` (set by `middleware/project-access.js` for a live-session-authenticated request) falling back to `req.session?.sessionId` (set instead when this router is mounted directly behind the older plain session-only middleware, which never populates `req.auth`) — a Copilot PR-review catch (2026-09-10): the fallback was previously missing, silently losing session attribution under that mount. `PUT`/`DELETE /file/storage-config` require Setup-tier project role (`requireSetup`, plan_project_roles.md; no-op passthrough when omitted, e.g. tests constructing the router directly); every other route (including `GET /file/storage-config`) stays open to any project member.
