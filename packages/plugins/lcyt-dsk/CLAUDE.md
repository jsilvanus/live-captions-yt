...
**Auth (`plan_authentication_refactor.md`):** template/viewport/image routes take JWT Bearer only — the `X-API-Key`-accepting `middleware/editor-auth.js` (`createEditorAuth`/`editorAuthOrBearer`) that used to sit in front of them has been removed. `createDskRouters()`'s `auth` param (the composition root's `scopedAuth('dsk')`, a project-access JWT check) is passed straight through to `createDskTemplatesRouter`/`createDskViewportsRouter`/`createImagesRouter` with no wrapper. `DskEditorPage.jsx` (lcyt-web) now sends `Authorization: Bearer <projectAccessToken>` like every other already-migrated page, so `requireSetup()` in `routes/dsk-templates.js`/`routes/dsk-viewports.js` no longer carries an `authKind === 'apikey'` exemption — every DSK Setup-tier write goes through the same real per-user role check as any other Setup-tier route.

`routes/dsk-templates.js` also has a second, separate local gate — `requireProduction()` — for its six live graphics-operate routes (`.../templates/:id/activate`, `/template`, `/broadcast`, `/graphics`, `/renderer/start`, `/renderer/stop`), assigned to the `'production'` tier (2026-09-10, `plan_project_roles.md`'s previously-undecided "/graphics page tier" gap). Unlike `requireSetup()`, it deliberately fails **open** when `req.user?.userId` is absent: `DskControlPage.jsx`'s sidebar mode (`/graphics/control`) authenticates with a plain caption-session JWT that carries no per-user identity at all, and must keep working. Once a real per-user project-access JWT is present — `DskControlPage.jsx`'s standalone mode (`/dsk-control/:key`) reusing an already-logged-in tab's persisted token — it fails closed exactly like `requireSetup()`, requiring operator+.

**Per-viewport stream config (Phase 4, in `display_settings_json.stream`):** `{ enabled, mode: 'composite'|'standalone', outputDimensions: { width, height }, pushUrls: [{ url, enabled }], chromaKey: { enabled, color, similarity, blend } }`, sanitized by `sanitizeStreamConfig` (rtmp(s):// push URLs only, clamped). Invariant: **at most one `composite` viewport per key** — `demoteOtherCompositeViewports()` demotes siblings on write. The `stream` object is **server-only**: `publicDisplaySettings()` strips it (pushUrls can carry stream keys) so `/viewports/public` never exposes it. Renderer consumption (per-viewport Chromium page-capture → `<key>__<viewport>` RTMP + tee push targets) uses `outputDimensions` as an optional override for the streamed output size; if omitted, the viewport's own width/height are used.
