...
Each of the four dep groups (`captionTargets`, `production`, `agent`, `assets`) is optional — `createToolRegistry` only builds the tool groups whose deps were actually provided, useful for tests that only care about one group.
...
**Tests:** `test/tool-registry.test.js` (registry assembly, per-tool-group fake deps, annotation coverage, `callTool` error paths), `test/in-process-bridge.test.js` (real MCP `Server`/`Client` round-trip via `InMemoryTransport` — `tools/list` parity, `_apiKey` scoping/stripping, error surfacing as a real MCP tool error not a thrown exception).
