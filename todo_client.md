...
- [ ] M1-1: Add `packages/lcyt-web/` to root `package.json` workspaces array
- [ ] M1-2: Create `packages/lcyt-web/package.json`
  - name: `lcyt-web`, private: true
  - devDependencies: `vite`
  - dependencies: `lcyt` (workspace)
  - scripts: `dev`, `build`, `preview`
- [ ] M1-3: Create `packages/lcyt-web/vite.config.js`
  - Resolve `lcyt` from workspace (`../lcyt`)
  - Dev server proxy: `/live`, `/captions`, `/sync` → `http://localhost:3000`
- [ ] M1-4: Create `packages/lcyt-web/index.html`
  - Semantic structure: `<header>`, `<main>`, `<footer>`, `<div id="app">`
  - `<script type="module" src="/src/main.js">`
- [ ] M1-5: Create `src/styles/reset.css` (box-sizing, normalize)
- [ ] M1-6: Create `src/styles/layout.css`
  - CSS grid: header fixed top, footer fixed bottom, main fills rest
  - Main splits into two columns (60% left, 40% right)
- [ ] M1-7: Create `src/styles/components.css` (placeholder, fill out per milestone)
- [ ] M1-8: Create `src/session.js`
...
- [ ] M1-9: Create `src/ui/settings-modal.js`
  ...
  - `open()` / `close()` methods
- [ ] M1-10: Create `src/ui/status-bar.js`
...
- [ ] M1-11: Create `src/main.js` — bootstrap: import styles, instantiate components, wire together
- [ ] M1-12: Add `"web": "npm run dev -w packages/lcyt-web"` and `"build:web": "npm run build -w packages/lcyt-web"` to root `package.json` scripts
...
- [ ] M2-2: Create `src/ui/drop-zone.js`
...
- [ ] M2-3: Create `src/ui/file-tabs.js`
...
- [ ] M2-4: Create `src/ui/caption-view.js`
...
- [ ] M2-5: Wire drop-zone and caption-view into `main.js`
...
- [ ] M2-6: Add styles for drop-zone, file-tabs, caption-view to `components.css`
...
- [ ] M3-2: Create `src/ui/input-bar.js`
...
- [ ] M3-3: Create `src/ui/sent-panel.js`
...
- [ ] M3-4: Wire sequence update in `status-bar.js`
  - Listen to `lcyt:sequence-updated`, update `Seq: <n>` display
- [ ] M3-5: Add error display to `status-bar.js`
...
- [ ] M3-6: Sent-panel empty state: "No captions sent yet" placeholder
- [ ] M3-7: Style input-bar: full-width, fixed at bottom, input fills available space, button on right
- [ ] M3-8: Style sent-panel: monospace font, seq column fixed width, time column fixed width, text wraps
...
- [ ] M4-1: Expand `src/ui/settings-modal.js` to `<dialog>` with tabbed sections
...
- [ ] M4-7: Keyboard shortcut: `Ctrl+,` / `Cmd+,` → open settings modal
- [ ] M4-8: `Escape` key closes settings modal (native `<dialog>` cancel event)
...
- [ ] M5-1: Keyboard navigation in caption-view (when input not focused)
...
- [ ] M5-2: "Sent" flash animation on active line after sending
...
- [ ] M5-4: Responsive layout below 768px
...
- [ ] M5-9: Serve from `lcyt-backend` (optional, low-effort)
...
- [ ] P2-M1-1: Add "Audio" section to settings modal (new tab)
- [ ] P2-M1-2: Enumerate audio devices via `navigator.mediaDevices.enumerateDevices()`
  - Filter to `audioinput` devices
  - Populate `<select>` with device labels + deviceIds
- [ ] P2-M1-3: "Request Microphone Permission" button
  - Call `getUserMedia({ audio: true })`, stop tracks immediately after
  - Update permission status display: Granted / Denied / Prompt
- [ ] P2-M1-4: Persist selected `deviceId` to `localStorage` as `lcyt-audio-device`
- [ ] P2-M1-5: Live audio level meter component (`src/ui/audio-meter.js`)
  - `AnalyserNode` → `getByteTimeDomainData()` → canvas bar render at 60fps
  - Color: green (low) → yellow (medium) → red (clipping)
  - Show only when mic is actively captured
- [ ] P2-M1-6: Handle `devicechange` event — refresh device list without page reload
...
- [ ] P2-M4-7: Start/stop STT session controls
  - "Start Listening" button in audio settings tab → `capture.startCapture()` + `sttClient.connect()`
  - "Stop Listening" button → `capture.stopCapture()` + `sttClient.disconnect()`
  - Disable if session not connected
