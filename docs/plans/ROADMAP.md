...
| `plan_local_stt.md` | **Superseded/retired** — the in-repo copy (`python-packages/lcyt-stt/`, `docker/lcyt-stt/`) was removed on `chore/retire-lcyt-stt`; development moved to and was substantially extended in the sibling `liturgos-auditor` repo | No longer scheduled from this repo. Integrates via the *unchanged* `WhisperHttpAdapter` — deploy `liturgos-auditor` and point `WHISPER_HTTP_URL` at it (see `PORTS.md`). |
