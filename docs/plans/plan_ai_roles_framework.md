...
**Implementation status (2026-07-20):** `routes/roles-chat.js`'s `POST /:roleCode/message` handles all three roles identically and is fully built and mounted (`lcyt-backend/server.js`). On the frontend, `<AgentChatPanel>` is mounted for `dsk_designer` (`DskEditorPage.jsx`) and for Planner (`PlannerAssistPanel.jsx`) — but `dsk_designer` doesn't actually need `useGuidedAction`, since generating/editing a DSK template is a direct API call, not a dialog to drive. **Setup Assistant and Asset Control Assistant have zero frontend presence**: no chat panel is mounted in `SetupHubPage.jsx` or `AssetsPage.jsx`, and `useGuidedAction` — the primitive this plan calls out as "new frontend work regardless of backend design" — isn't implemented anywhere in `lcyt-web`. These two roles are backend-complete and frontend-unbuilt.
...
6. **Build order for the five `agentic_chat` roles — resolved by what actually shipped, 2026-07-20.** Planner, Graphics Editor Assistant, and Production Assistant are done end-to-end (backend + frontend where applicable). Setup Assistant and Asset Control Assistant's backend (`POST /roles/:roleCode/message`) is done, but their frontend — a mounted `<AgentChatPanel>` plus the `useGuidedAction` dialog-driving primitive — was never built (verified: no reference to either role code, or to `useGuidedAction`, anywhere in `lcyt-web`). This is the one concretely unbuilt, in-scope item left in this plan, not an open product question anymore.
