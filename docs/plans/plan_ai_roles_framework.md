...
status: in-progress
summary: "Extensible AI 'role' registry (ai_roles catalog + project_ai_role_configs) that replaces ad-hoc single-purpose AI config with a generic model+harness selection mechanism. Implemented: the catalog + config schema (in its ai_model_registry-amended form, provider_id FK from day one), both vision roles (Tracker/Describer — continuous_vision runtime, preview-JPEG frame source, OpenAI/Google/Anthropic vision adapters), the shared agentic_chat turn loop with its confirm/auto safety gate, all five roles' backend (POST /roles/:roleCode/message for Setup/Asset Control/Graphics Editor Assistant, Production Assistant's suggestion queue, POST /roles/planner/assist superseding /agent/generate-rundown|edit-rundown). Bridge-relayed provider support for agentic_chat/vision is done: the turn loop's resolveRoleProviderSettings()/invokeModelCall() and all three vision adapters (OpenAI/Google/Anthropic) dispatch through bridgeManager.sendCommand({type:'model_call'}) when a provider's bridge_instance_id is set, same as direct providers. The <AgentChatPanel> frontend is shipped for all five agentic_chat roles as of 2026-07-21: Planner (via PlannerAssistPanel), Graphics Editor Assistant/dsk_designer (DskEditorPage, dialog-free so it never needed guided-action), and now Setup Assistant / Asset Control Assistant via a new `<RoleAssistantPanel>` (`lcyt-web/src/components/agent/RoleAssistantPanel.jsx`) mounted in `SetupHubPage.jsx`/`AssetsPage.jsx`. The `useGuidedAction` dialog-driving primitive (`hooks/useGuidedAction.jsx` — `GuidedActionProvider`/`useGuidedActionTargets`/`useGuidedActionDispatcher`) is built and wired into Setup Assistant's three dialog-owning managers (`CaptionTargetsManager`/`CamerasManager`/`MixersManager`, each gaining `openAddPrefilled`/`openEditPrefilled`/`openDeleteConfirm` alongside their existing `openAdd`): a `confirm`-mode staged tool call from the backend opens the real Add/Edit/Delete dialog pre-filled, and the human still clicks that dialog's own submit button. **Not done:** Asset Control Assistant's tools (`asset.update`/`asset.delete`) have no registered guided-action target — they operate on `lcyt-dsk`'s image-layer asset library, which `AssetsPage.jsx` has no dialog for (a real page-ownership question, not a build gap — see CONSIDER.md); its chat panel still works for reads/plain-text proposals, just not dialog-driving. `encoders`/`bridges` Setup Hub cards also have no guided-action wiring (not in Setup Assistant's `available_tools`, so out of scope). Translation (still just a flagged future gap, not spec'd). Supersedes the 'AI Models — tracker/describer/assistant multi-role' gap noted in plan_team_org_backend.md's appendix."
related: plan/agent, plan/cues, plan/team_org_backend, plan/mcp, plan/ai_model_registry, plan/video_perception, plan/prod, plan/vertical_crop
---
...
> **Amended by `plan_video_perception.md` (draft, 2026-07-20):** Tracker/Describer are specced below as **project**-scoped (one preview-JPEG feed per project). A project with N `prod_cameras` needs N-way sampling/batching instead — that plan's job is camera-scoping this pair, not this plan's. Concretely: `project_ai_role_configs` gains an optional `camera_id` dimension for these two roles only (NULL = today's project-wide behavior, preserved as the default for single-camera projects), `VisionFrameFetcher` is parameterized per camera's own feed instead of the one project-wide preview endpoint, and `VisionRoleManager` batches/staggers polling across a project's cameras rather than running each independently at the same interval. Also amended: Describer's `harness_config.systemPromptOverride` (table below) splits into a fixed base template — owns the output-schema contract (`outputMode`/`jsonSchema`), never project-editable — plus a project-editable **venue-context block** (camera/preset labels from `plan_video_perception.md`'s camera metadata, venue vocabulary) that gets interpolated in. This is what that plan's AI observability/"prompt sculpting" page actually edits, rather than a raw prompt-string field. Neither change is built yet — see `plan_video_perception.md` for phasing. Production Assistant (below) also gains World State's `scene.*` topics as additional context once that service exists — no change to its trigger mechanism, just richer input on the same subscription pattern it already uses.
...
- **Graphics Editor Assistant** (`role_code: 'dsk_designer'`) — formally absorbs the existing `generateTemplate`/`editTemplate`/`suggestStyles` capability. Was informally previewed in the Setup Hub's AI models card under this user-facing name (`AiModelsSection.jsx`, added 2026-07-06) ahead of this plan being implemented; that card was disconnected dead-end plumbing and was deleted 2026-07-20 (ROADMAP.md Tier 0) — see `plan_ai_model_registry.md` Phase 3's note.
...
| Describer | `outputMode` (`'text'`\|`'json'`), `jsonSchema`, `systemPromptOverride`, `pollIntervalMs` | free text vs. structured description, and the schema when `outputMode: 'json'`. **Planned (`plan_video_perception.md`):** `systemPromptOverride` splits into a fixed base template + a project-editable venue-context block; both Tracker and Describer gain an optional per-camera scope instead of always polling the one project-wide preview feed. |
...
**Implementation status (2026-07-21):** `routes/roles-chat.js`'s `POST /:roleCode/message` handles all three roles identically and is fully built and mounted (`lcyt-backend/server.js`). On the frontend, `<AgentChatPanel>` is mounted for `dsk_designer` (`DskEditorPage.jsx`) and for Planner (`PlannerAssistPanel.jsx`) — `dsk_designer` doesn't need `useGuidedAction`, since generating/editing a DSK template is a direct API call, not a dialog to drive. **Setup Assistant and Asset Control Assistant now have frontend presence**: `<RoleAssistantPanel>` (a thin `POST /roles/:roleCode/message` wrapper around `<AgentChatPanel>`) is mounted in `SetupHubPage.jsx` and `AssetsPage.jsx`, each page wrapped in a `GuidedActionProvider`. `useGuidedAction.jsx` implements the primitive this plan calls out as "new frontend work regardless of backend design": `useGuidedActionTargets(handlers)` lets a dialog-owning component register a handler per tool name; `RoleAssistantPanel` calls `useGuidedActionDispatcher().dispatch(toolName, args)` for each `confirm`-mode `pendingActions` entry the backend returns, which opens/pre-fills the matching real dialog (a registered handler returns `true`; an unregistered tool name — e.g. Asset Control's, see above — surfaces as plain chat text instead of silently no-oping). Setup Assistant's three dialog-owning managers (`CaptionTargetsManager`, `CamerasManager`, `MixersManager`) register `create`/`update`/`delete` handlers; each gained `openAddPrefilled`/`openEditPrefilled`/`openDeleteConfirm` on its existing `useImperativeHandle`.
...
- **Planned (`plan_video_perception.md`):** World State's `scene.*` topics (active speaker, best framing, segment change) as additional context on the same trigger mechanism — no new subscription pattern, just richer facts to reason over than raw per-role events alone.
...
6. **Build order for the five `agentic_chat` roles — resolved by what actually shipped, 2026-07-21.** All five roles are now done end-to-end (backend + frontend where applicable): Planner, Graphics Editor Assistant, and Production Assistant as before, plus Setup Assistant and Asset Control Assistant's `<RoleAssistantPanel>` + `useGuidedAction` primitive. The one remaining concrete gap is narrower than "no frontend" — it's that Asset Control Assistant's specific tools have no dialog to drive on `AssetsPage.jsx` (see CONSIDER.md), a page-ownership question rather than missing plumbing.
