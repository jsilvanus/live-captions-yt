...
status: in-progress
summary: "Design for a real multi-project Team/Org data model (organizations, org_members, api_keys.org_id) to replace the /team 'Coming soon' placeholder. Resolves the org-vs-project access precedence question with a baseline-plus-override model. Implemented: schema (`organizations`, `org_members`, `api_keys.org_id`), full org CRUD + membership routes (`/orgs/*`), and the `/team` frontend page (see `plan_profile_team_admin_reconciliation.md`) — but `org_members.role` shipped as a 5-tier `owner/admin/editor/operator/viewer` vocabulary (`ROLE_ORDER` in `src/routes/orgs.js`), not this doc's proposed 3-tier `owner/admin/member`, and the `restricted`-project escape hatch below was never added. Remaining, confirmed still true as of 2026-07-20: the `getEffectiveProjectAccessLevel()` org-baseline-plus-project-override access resolver below — no function by this or any other name exists anywhere in the backend (`src/routes/project-features.js`, `src/routes/keys.js`, `src/routes/device-roles.js`, and `src/middleware/project-access.js` all call the raw `getMemberAccessLevel()` directly) — and the mechanical sweep of existing per-project auth checks onto it. Org membership today only grants access to org-level surfaces (org CRUD/members/audit/usage/features, per-org connector SSRF rules); it grants zero baseline access to any individual member project's resources (captions, DSK, variables, cues, STT, targets, translation, etc.) — i.e. the shipped behavior matches this doc's rejected 'Option 1', not the recommended 'Option 2'."
---
