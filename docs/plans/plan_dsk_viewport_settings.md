...
## Phase 4 — Multi-renderer: per-viewport streams with scoped graphics ✅ (implemented 2026-07-11; renderer capture needs live validation)
...
> **Renderer increment implemented 2026-07-11 (pure helpers tested; Chromium/ffmpeg wiring needs live validation):** `src/renderer-helpers.js` — `viewportPageUrl`, `resolveCaptureDimensions`, `resolveCaptureBackground` (transparent + chromaKey → key color; else black + warning), `buildViewportOutputs` (local `<key>__<viewport>` leg + enabled rtmp push targets → ffmpeg tee), all unit-tested. `renderer.js` gains `startViewportStream`/`stopViewportStream`/`listViewportStreams` keyed by `<apiKey>::<viewport>` (own Chromium page capturing `/dsk/<slug>/<viewport>` → tee output), alongside the untouched legacy per-key template renderer; the Chromium-crash recovery replays viewport streams with their original opts. `renderer/start|stop` accept `{ viewport? }`; `getStatus` reports `viewportStreams`. Viewports-page Streaming section gains Start/Stop controls. The actual page-capture + ffmpeg spawn are integration-only and need a live browser + ffmpeg + RTMP + served SPA to confirm end-to-end.
