...
- Apply the schema diff in "Amendment to `project_ai_role_configs`" above when that plan is implemented (not before — it doesn't exist yet).
- Rewrite `AgentEngine._resolveApiSettings()` (`agent-engine.js:269-282`) to take a `provider_id` instead of the current `ai_config` row, resolve it via `providerRegistry.getProvider()`, and branch on `kind`/`bridge_instance_id` to build the actual request (direct HTTP vs. `bridgeManager.sendCommand({type:'model_call', ...})`).
- Role-config UI's model picker becomes `GET /ai/providers/:id/models` for an `ollama`-kind provider (falling back to free text only if that specific provider hasn't been discovered yet / has zero rows); an `api`-kind provider is always a free-text `model_name` field, by design — there's no catalog to pick from.
