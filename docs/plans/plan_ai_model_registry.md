...
- **Not done — frontend, outside this plan's backend scope.** Role-config UI's model picker still needs to become `GET /ai/providers/:id/models` for an `ollama`-kind provider (falling back to free text only if that specific provider hasn't been discovered yet / has zero rows); an `api`-kind provider stays a free-text `model_name` field, by design — there's no catalog to pick from. No `lcyt-web` component calls `/ai/providers` yet (re-checked via grep, 2026-07-20) — today a role's `provider_id`/`model_name` can only be set by a direct API call, not through the Setup Hub UI.
  ...
  **The Setup Hub's old "AI models" card was not this — and has been removed.** `AiModelsSection.jsx` (Setup Hub, `id="ai-models"`) and its backend (`routes/ai-models.js`, formerly mounted at `/ai/models` in `server.js`) were a separate CRUD path: a standalone `ai_model_configs` table (`ai-models.js`, `runAiModelMigrations`) with its own `provider: 'api'|'ollama'` + free-text `model_name`/`api_url`/`api_key_ref` columns, hardcoded to `role_code: 'assistant'` only in the UI. It never referenced `ai_providers`/`ai_provider_models`/`ai_provider_grants` at all, had no discovery persistence, and — critically — `getAiModelConfig()` had zero call sites outside its own module: nothing in `agentic-turn.js`, `production-assistant.js`, or any role route ever read from `ai_model_configs`. Deleted whole (component, route, db table code, and the `/admin/ai-models` page's usage of it — that page now hosts only `McpAccessSection`) as ROADMAP.md Tier 0, 2026-07-20, per the "delete it" branch of that item's own decision — see CONSIDER.md's (resolved) entry. Phase 3's frontend model-picker described above is still unbuilt and now starts from a clean slate, with no stale card to fold in or repurpose.
