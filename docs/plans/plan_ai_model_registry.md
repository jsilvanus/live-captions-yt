...
status: implemented
summary: "ai_providers / ai_provider_models / ai_provider_grants: a registry of model *sources* (cloud API, self-hosted Ollama, in-process 'deer' runtimes) that sits underneath plan_ai_roles_framework.md's per-role config. Phase 1 (registry core: schema, CRUD, grants, direct-reachable discovery), Phase 2 (bridge-relayed providers: model_call bridge command with a per-call timeout override, Dockerized bridge+Ollama deployment mode), and Phase 3 (wiring project_ai_role_configs' provider_id through to the agentic_chat turn loop and all three vision adapters for *inference*, plus the Setup Hub role-config UI) are implemented — resolveRoleProviderSettings()/invokeModelCall() and the OpenAI/Google/Anthropic vision adapters all dispatch through bridgeManager.sendCommand({type:'model_call'}) for a bridge-relayed provider, same as direct ones, and `lcyt-web`'s Setup Hub `AiRoleModelsSection.jsx` (`id=\"ai-roles\"`) lets a project pick a provider + model per `agentic_chat` role and saves via the existing `PUT /roles/:roleCode/config` route — a fresh component, not a repurposing of the old disconnected `AiModelsSection.jsx`/`ai_model_configs` CRUD path (deleted whole, ROADMAP.md Tier 0, 2026-07-20; see CONSIDER.md). Phase 4 ('deer' in-process runtimes) remains unscoped, pending inspection of the actual jsilvanus/deer package APIs. Amends project_ai_role_configs to reference this registry instead of embedding provider/credential fields per role; supersedes plan_agent.md's Phase 8 (Local Model Support) stub."
...
- **Done, 2026-07-20.** Role-config UI's model picker: `packages/lcyt-web/src/components/setup-hub/AiRoleModelsSection.jsx`, a new Setup Hub card (`id="ai-roles"`, mounted in `SetupHubPage.jsx`'s "AI & integrations" group alongside `McpAccessSection`/`ConnectorsSection`). Lists every `agentic_chat`-runtime-kind role from `GET /roles/catalog`, this project's visible providers from `GET /ai/providers`, and each role's current config from `GET /roles/:roleCode/config`; a per-role settings dialog picks a `provider_id` and, for an `ollama`-kind provider, a model from `GET /ai/providers/:id/models` (falling back to a free-text field with a note when that provider has zero catalog rows yet — hasn't been discovered, or is freshly created); an `api`-kind provider is always free-text `model_name`, by design (no catalog exists to validate against — see "Discovery Mechanics" above). A `deer`-kind provider is listed but disabled in the picker ("coming soon"), matching this plan's "greyed-out, not omitted" convention. Saves via the existing `PUT /roles/:roleCode/config` route (`routes/roles.js`) — no backend changes were needed for this phase, `GET /ai/providers/:id/models` (`routes/ai-providers-project.js`) already existed and worked. A row-level toggle switch does a quick enable/disable without opening the dialog (`PUT` with just `{enabled}`), same convention as other Setup Hub cards' item rows. Tests: `packages/lcyt-web/test/components/AiRoleModelsSection.test.jsx` (Vitest — role filtering, summary line, quick toggle, dialog save for an `api`-kind provider, discovered-model dropdown for an `ollama`-kind provider).

  **The Setup Hub's old "AI models" card was not this — and was removed first.** `AiModelsSection.jsx` (Setup Hub, `id="ai-models"`) and its backend (`routes/ai-models.js`, formerly mounted at `/ai/models` in `server.js`) were a separate CRUD path: a standalone `ai_model_configs` table (`ai-models.js`, `runAiModelMigrations`) with its own `provider: 'api'|'ollama'` + free-text `model_name`/`api_url`/`api_key_ref` columns, hardcoded to `role_code: 'assistant'` only in the UI. It never referenced `ai_providers`/`ai_provider_models`/`ai_provider_grants` at all, had no discovery persistence, and — critically — `getAiModelConfig()` had zero call sites outside its own module: nothing in `agentic-turn.js`, `production-assistant.js`, or any role route ever read from `ai_model_configs`. Deleted whole (component, route, db table code, and the `/admin/ai-models` page's usage of it — that page now hosts only `McpAccessSection`) as ROADMAP.md Tier 0, 2026-07-20, per the "delete it" branch of that item's own decision — see CONSIDER.md's (resolved) entry. `AiRoleModelsSection.jsx` above is a genuinely new component built against the real registry, not a repurposing of that shell.
