...
| [plan_asset_backends.md](plans/plan_asset_backends.md) | Asset Backends — Server-Side Rundowns + Graphics-Editor Thumbnails | Backs two Assets placeholders: rundowns as `caption_files` (`type='rundown'`) via a new user-facing file authoring API + planner Save/Open; thumbnails as still-PNG renders of DSK templates (template-ref + cached PNG) in a new `thumbnails` table. |
| [plan_recording_vod.md](plans/plan_recording_vod.md) | Recording & VOD Pipeline — Stored Videos from Broadcasts | Opt-in-per-broadcast recording via MediaMTX native record (`patchPath`) → HLS VOD on S3, indexed in a `videos` table keyed to the broadcast; worker-daemon ffmpeg recorder as a phase-2 alternative behind a swappable interface. |
