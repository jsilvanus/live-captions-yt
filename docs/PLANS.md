...
| [plan_broadcasts.md](plans/plan_broadcasts.md) | Broadcasts — First-Class Intra-Project Broadcast Entity | Adds a `broadcasts` entity (a project casts many times): lifecycle, scheduling, `broadcast_assets` linkage of reusable assets, and `broadcast_id` on sessions/stats/caption files so produced content + YouTube casts attach to the broadcast that made them. |
