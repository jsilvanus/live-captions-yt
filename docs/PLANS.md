...
| [plan_cues.md](plans/plan_cues.md) | Cue Engine Enhanced Capabilities | Phases 1–8 fully implemented: inline cues, modifiers, fuzzy matching, sound detection, semantic/event cues. Phase 10 (cue rules editor) implemented 2026-07-18, scoped to today's simple rule types (phrase/regex/section/fuzzy) — a `CuesManager` core reused by the standalone `/cues` page and, as of the same day, an embedded "Cues" tab in the Planner's right-column `PlannerAssistPanel` (alongside a "Actions" tab for named actions) above the AI assistant chat. **Not done:** Phase 8.5 (inline↔backend cue sync gap), Phase 9 (composite trees + named conditions, `/cues/defs`), the editor's composite-tree extension (blocked on Phase 9), and multi-modal scene understanding — all still planned. |
