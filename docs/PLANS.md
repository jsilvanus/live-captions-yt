...
| [plan_stt_client.md](plans/plan_stt_client.md) | Client-side Speech-to-Text (STT) | Browser-based speech capture in lcyt-web: WebKit (Web Speech API) and Google Cloud STT engines, client-side VAD, translation pipeline, MCP speech sessions, and embed widget. | |
...
| [plan_translate_client.md](plans/plan_translate_client.md) | Client-side Caption Translation Pipeline | Client-side real-time translation before sending: vendor abstraction (MyMemory, Google, DeepL, LibreTranslate), multi-target routing, per-language viewer display, HLS subtitle sidecar, and local/backend file writing. | |
...
| [plan_ui.md](plans/plan_ui.md) | Frontend & UI Plans | Four iterations: v1 (two-column layout, superseded), v2 (sidebar navigation + dashboard, core implemented), v3 (component split, completed), v4 (two-phase login + feature-based UI, implemented). Remaining P1: empty-state guidance (unsaved work protection). | |
| [plan_stt_server.md](plans/plan_stt_server.md) | Server-side Speech-to-Text (STT) | Phase 1 implemented: HlsSegmentFetcher, GoogleSttAdapter, WhisperHttpAdapter, OpenAiAdapter, SttManager, /stt routes. Phases 2–4 (gRPC streaming, RTMP/WHEP fallback enhancements) remain. | |
...
| File | Title | Summary | Supersedes |
|---|---|---|---|
| [plan_translate_server.md](plans/plan_translate_server.md) | Server-Side Translation Plugin (`lcyt-translate`) | Exploratory plan for server-side translation to close the STT and CLI translation gap: vendor adapters (MyMemory, Google, DeepL, LibreTranslate), per-key DB config, injection into captions.js and SttManager. | |
